...
(defn alterar-using-put3-with-http-info
  ...
  ([id nome tipo ] (alterar-using-put3-with-http-info id nome tipo nil))
...
(defn alterar-using-put3
  "Atualiza os dados de uma determinada Pessoa
  Este m̩todo permite que seja alterado na base do emissor um registro de determinada Pessoa."
  ([id nome tipo ] (alterar-using-put3 id nome tipo nil))
  ([id nome tipo optional-params]
   (:data (alterar-using-put3-with-http-info id nome tipo optional-params))))
...
(defn consultar-using-get11-with-http-info
  ...
  ([] (consultar-using-get11-with-http-info nil))
...
(defn consultar-using-get11
  "Lista os tipos de boletos do emissor
  Este recurso permite que sejam listados os tipos de boletos existentes na base de dados do emissor."
  ([] (consultar-using-get11 nil))
  ([optional-params]
   (:data (consultar-using-get11-with-http-info optional-params))))
...
(defn salvar-using-post3-with-http-info
  ...
  ([nome tipo ] (salvar-using-post3-with-http-info nome tipo nil))
...
(defn salvar-using-post3
  "Realiza o cadastro de um nova Pessoa
  Este m̩todo permite que seja cadastrado uma nova Pessoa na base de dados do Emissor."
  ([nome tipo ] (salvar-using-post3 nome tipo nil))
  ([nome tipo optional-params]
   (:data (salvar-using-post3-with-http-info nome tipo optional-params))))
